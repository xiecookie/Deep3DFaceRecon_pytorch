...
def main(rank, world_size, train_opt):
    ...
    for epoch in range(train_opt.epoch_count,
                       train_opt.n_epochs + 1):  # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, train_data in enumerate(train_dataset):  # inner loop within one epoch
            ...
            if rank == 0 and (
                    total_iters == batch_size or total_iters % train_opt.display_freq == 0):  # display images on visdom and save images to a HTML file
            ...
            if rank == 0 and (
                    total_iters == batch_size or total_iters % train_opt.print_freq == 0):  # print training losses and save logging information to the disk
            ...
            if total_iters == batch_size or total_iters % train_opt.evaluation_freq == 0:
                with torch.no_grad():
                    ...
                    if rank == 0:
                        visualizer.print_current_losses(epoch, epoch_iter, losses_avg, eval_time, t_data,
                                                        dataset='val')  # visualize training results
